...
[ -f /mnt/etc/ssh/ssh_host_ed25519_key ] || run ssh-keygen -t ed25519 -f /mnt/etc/ssh/ssh_host_ed25519_key -N ""
[ -f /mnt/etc/ssh/ssh_host_rsa_key ] || run ssh-keygen -t rsa -b 4096 -f /mnt/etc/ssh/ssh_host_rsa_key -N ""

###############
#   INSTALL   #
###############

# disable setting root password post installation if we have it set in the config
# ...which we did, but i'll keep it just in case
